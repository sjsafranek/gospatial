...
// @param share - customer to give access
// @param apikey
// @return json
/*=======================================*/
func ShareLayerHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Get params
	apikey := r.FormValue("apikey")
	share_apikey := r.FormValue("share")
	...
	// Check for apikey in request
	if apikey == "" {
		Error.Println(r.RemoteAddr, "PUT /api/v1/layer [401]")
		http.Error(w, "unauthorized", http.StatusUnauthorized)
		return
	}
	...
	customer, err := DB.getCustomer(apikey)
	if err != nil {
		Warning.Println(r.RemoteAddr, "PUT /api/v1/layer/ [404]")
		http.Error(w, err.Error(), http.StatusNotFound)
		return
	}

	// Get customer from database
	share_customer, err := DB.getCustomer(share_apikey)
	if err != nil {
		Warning.Println(r.RemoteAddr, "PUT /api/v1/layer/ [404]")
		http.Error(w, err.Error(), http.StatusNotFound)
		return
	...
	// Add datasource uuid to customer
	share_customer.Datasources = append(share_customer.Datasources, ds)
	DB.insertCustomer(share_customer)
